    ...
    class Solution {
        public int eraseOverlapIntervals(int[][] intervals) {
            ...
            for (int[] interval : intervals) {
                int diff = interval[1] - interval[0];
                if (map.get(diff) == null) map.put(diff, new PriorityQueue<int[]>((p, q) -> {
                    return p[0] < q[0] ? p[0] - q[0] : p[1] - q[1];
                }));
                map.get(diff).add(interval);
            }

            int t = 0;
            ArrayList<int[]> result = new ArrayList<>();
            Map<String, ArrayList<String>> overlappings = new HashMap<>();
            for (Map.Entry<Integer, PriorityQueue<int[]>> entry : map.entrySet()) {
                PriorityQueue<int[]> list = entry.getValue();
                for (int[] interval : list) {
                    boolean overlapping = false;
                    for (int[] cs : result) {
                        if (interval[0] >= cs[1] || interval[1] <= cs[0]) {
                        } else {
                            if (overlappings.get(cs[0] + "-" + cs[1]) == null) overlappings.put(
                                    cs[0] + "-" + cs[1],
                                    new ArrayList<>()
                            );
                            overlappings.get(cs[0] + "-" + cs[1]).add(interval[0] + "-" + interval[1]);
                            overlapping = true;
                            break;
                        }
                    }
